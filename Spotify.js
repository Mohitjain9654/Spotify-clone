...
async function getSongs(folder) {
  ...
  for (const song of songs) {
    const imagePath = `http://127.0.0.1:5500/${folder}/${song.replaceAll(".mp3", ".jpeg").replaceAll(".mp4", ".jpeg")}`;
    songUL.innerHTML += `<li>
...
async function displayAlbums() {
  ...
  let playlist = await fetch(`http://127.0.0.1:5500/songs/`);
  ...
  //       `http://127.0.0.1:5500/songs/${playlistfolder}/info.json`
  //     );
